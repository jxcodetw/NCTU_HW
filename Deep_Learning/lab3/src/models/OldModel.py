...
class OldModel(CaptionModel):
	...
	def get_logprobs_state(self, it, tmp_fc_feats, tmp_att_feats, state):
		...
		output, state, atten = self.core(xt, tmp_fc_feats, tmp_att_feats, state)
	...
	def sample_beam(self, fc_feats, att_feats, opt={}):
		...
		for k in range(batch_size):
			...
			self.done_beams[k], attens = self.beam_search(state, logprobs, tmp_fc_feats, tmp_att_feats, opt=opt)
			seq[:, k] = self.done_beams[k][0]['seq'] # the first beam has highest cumulative score
			seqLogprobs[:, k] = self.done_beams[k][0]['logps']


			# UP = nn.Upsample(size=(512, 512), mode='bilinear')
			# img_src = img_srcs[k]
			# # for idx, atten in enumerate(attens):
			# for idx, atten in enumerate(self.done_beams[k][0]['attens']):
			# 	filename = 'atten/{}.{}.png'.format(k, idx)
			# 	atten = UP(atten.view(1, 1, 14, 14)).view(1, 512, 512).cpu().data.numpy()
			# 	atten = atten.transpose(1, 2, 0)
			# 	atten_min = np.min(atten)
			# 	atten_max = np.max(atten)
			# 	_div = atten_max - atten_min

			# 	if _div == 0 and atten_max == 0 and atten_min == 0:
			# 		continue
			# 	else:
			# 		atten = (atten - np.min(atten)) / (np.max(atten)-np.min(atten))
			# 		atten = gaussian_filter(atten, sigma=20)
			# 		atten = (atten - np.min(atten)) / (np.max(atten)-np.min(atten))
			# 		combine = img_src * atten
			# 		plt.imsave(filename, combine, format='png', cmap='gray')


		# return the samples and their log likelihoods
		return seq.transpose(0, 1), self.done_beams, seqLogprobs.transpose(0, 1)
